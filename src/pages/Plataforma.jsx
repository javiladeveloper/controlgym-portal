      ...
      {data && (
        <>
          ...
          <div className="mt-[15px] grid grid-cols-4 gap-[15px]">
            <StatCard label="Leads del mes" value={k.leads_mes ?? 0} delta="captados por las páginas" />
            <div className="col-span-2 rounded-card border border-line bg-white p-[17px]">
              <div className="text-[11px] font-extrabold uppercase tracking-[0.6px] text-muted">Volumen facturado por los gyms (mes)</div>
              ...
              <div className="mt-0.5 text-[12px] font-semibold text-muted">
                base para tu futura comisión por pagos online
              </div>
            </div>
            <div className="rounded-card border border-line bg-navy p-[17px]">
              <div className="text-[11px] font-extrabold uppercase tracking-[0.6px] text-faint">Ingreso potencial SaaS</div>
              <div className="mt-1.5 text-[27px] font-extrabold text-white">{money((k.empresas ?? 0) * 99, 'PEN')}</div>
              <div className="mt-0.5 text-[12px] font-semibold text-faint">a S/99 por gym/mes</div>
            </div>
          </div>
          ...
          <Card className="mt-[15px] overflow-hidden">
            ...
            <div className="grid grid-cols-[1.8fr_1.1fr_0.6fr_0.7fr_0.8fr_1fr_1fr] items-center gap-3 bg-surface px-5 py-[11px] text-[11px] font-extrabold uppercase tracking-[0.6px] text-muted">
              <div>Gimnasio</div><div>Categoría</div><div>Sedes</div><div>Socios</div><div>Leads mes</div><div>Ingresos mes</div><div>Página</div>
            </div>
            {empresas.map((e) => {
              const cat = CAT_COLOR[e.categoria] || { bg: T.surface, color: T.muted }
              return (
                <div key={e.id} className="grid grid-cols-[1.8fr_1.1fr_0.6fr_0.7fr_0.8fr_1fr_1fr] items-center gap-3 border-t border-line2 px-5 py-3 hover:bg-[#FAFBFC]">
                  <div>
                    <div className="text-[13.5px] font-extrabold">{e.nombre}</div>
                    <div className="text-[11px] font-semibold text-muted">
                      {e.slug}.fitcorecenter.com · desde {new Date(e.created_at).toLocaleDateString('es-PE', { day: '2-digit', month: 'short', year: '2-digit' })}
                    </div>
                  </div>
                  <div><Badge bg={cat.bg} color={cat.color}>{e.categoria}</Badge></div>
                  <div className="text-[13px] font-bold">{e.sedes}</div>
