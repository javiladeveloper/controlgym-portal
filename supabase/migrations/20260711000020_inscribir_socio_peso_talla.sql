-- Fix: "le puse talla y peso y no guardo" -- el peso/talla se guardaban con un
-- UPDATE posterior desde el navegador que podia no afectar filas (RLS) sin
-- avisar. Ahora inscribir_socio los recibe como parametros y los guarda en el
-- MISMO insert del socio; con eso su llamada interna a asignar_plan_automatico
-- ya encuentra peso/talla y el plan sale en un solo viaje.
-- Nota: se agregan parametros con default => hay que botar la firma anterior
-- para no dejar dos overloads ambiguos en PostgREST.

drop function if exists public.inscribir_socio(uuid, text, text, text, text, date, text, uuid, uuid, uuid, text, jsonb, numeric, numeric, uuid);
