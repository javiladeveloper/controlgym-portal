-- registrar_empresa: sembrar el equipamiento típico de la sala al crear el gym.
--
-- Cambio respecto de la versión anterior: se agrega una sola línea
--
--     perform public.sembrar_equipamiento_sede(v_sede);
--
-- al final del bloque de categorías CON sala (fitness / clases / niños), justo
-- antes del `return`. Un personal_trainer no tiene sala de máquinas, así que
-- queda fuera a propósito.
--
-- Por qué: el generador de rutinas filtra los ejercicios por el equipo cargado en
-- la sede. Sin máquinas, un gym nuevo solo podía usar peso corporal (325 de 1.369
-- ejercicios = 24%). Con la siembra pasa a 1.241 (91%) y las rutinas del primer
-- día ya usan poleas, barras y máquinas.
-- Ver 20260720110000_equipamiento_gym_nuevo.sql.
--
-- La función se reemplaza completa (Postgres no permite parchear un fragmento);
-- el resto del cuerpo es idéntico al de la versión previa.
create or replace function public.registrar_empresa(
  p_nombre text, p_slug text, p_categoria_codigo text default 'fitness',
...
begin
  ...
  -- ¿Ya usó su mes gratis como CREADOR de otro negocio?
  select exists (
  ...
  if p_categoria_codigo = 'personal_trainer' then
    ...
    -- ← EL CAMBIO: equipamiento típico de la sala. Sin esto el generador de
    -- rutinas solo ve ejercicios de peso corporal (24% del catálogo) y las
    -- rutinas salen pobres desde el día uno. El dueño lo ajusta en el onboarding
    -- (ajustar_equipamiento_empresa) o en Máquinas.
    perform public.sembrar_equipamiento_sede(v_sede);
  end if;
