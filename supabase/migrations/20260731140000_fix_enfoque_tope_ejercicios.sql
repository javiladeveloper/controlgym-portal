-- Fix: el enfoque muscular generaba días de 8-9 ejercicios.
--
-- REPORTADO por el owner probando (avanzado / 5 días / tren inferior):
--   Empuje  → 3 ejercicios
--   Pierna  → 8 ejercicios   ← demasiado
--   Pierna+core → 9          ← demasiado
--
-- CAUSA: el enfoque sumaba +1 ejercicio POR CADA músculo del día, sin techo.
-- Un día "Pierna + core" tiene 5 músculos (quads, hamstrings, glutes, abs,
-- calves) → 5 × (1+1) = 10. Y los días del tren no enfatizado bajaban a 3,
-- quedando la rutina desbalanceada en ambos sentidos.
--
-- FIX: techo de ejercicios POR DÍA (no por músculo). Se reparte ese cupo entre
-- los músculos del día, dando prioridad a los del tren enfatizado. Rango sano:
-- 4-6 ejercicios por sesión (más de eso es sobreentrenamiento para la mayoría;
-- el volumen se controla con las series, que ya suben por nivel).
--
-- Techo por nivel: principiante 4, intermedio 5, avanzado 6.
