-- ============================================================================
-- 70 · Inscripción con pago inicial parcial
-- p_monto_inicial: lo que el socio paga HOY (null = el total). El resto queda
-- como saldo en la membresía (monto_pagado) y se cobra con abonar_membresia.
-- En promos de grupo (2x1/NxM) se cobra completo: un pago compartido en
-- partes entre varias personas no cuadra en caja.
-- ============================================================================

drop function if exists public.inscribir_socio(uuid,text,text,text,text,date,text,uuid,uuid,uuid,text,jsonb);
...
create or replace function public.inscribir_socio(
  ...
  p_invitados jsonb default null,
...
grant execute on function public.inscribir_socio(uuid,text,text,text,text,date,text,uuid,uuid,uuid,text,jsonb,numeric) to authenticated;
