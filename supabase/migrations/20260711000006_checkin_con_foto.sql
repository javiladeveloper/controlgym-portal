-- Foto del socio al CHECK-IN (verificacion de identidad, gyms sin huellero):
-- ambas RPCs devuelven foto_url SOLO si la foto esta aprobada por recepcion.
...
declare
  ...
  v_foto text;
begin
  select empresa_id, nombre, case when foto_estado = 'aprobada' then foto_url end
    into v_empresa, v_nombre, v_foto from public.socio where id = p_socio_id;
  ...
  -- foto_url solo si esta APROBADA: recepcion corrobora identidad (anti-suplantacion).
  return jsonb_build_object('resultado', v_resultado, 'motivo', v_motivo, 'socio', v_nombre, 'foto_url', v_foto);
end;
$function$;
...
begin
  ...
  if v_formato = 'fc1' then
    ...
    select s.usuario_id, s.sede_id, s.nombre, case when s.foto_estado = 'aprobada' then s.foto_url end
      into v_uid, v_sede_id, v_nombre, v_foto
    ...
    select s.id, s.sede_id, s.nombre, case when s.foto_estado = 'aprobada' then s.foto_url end
      into v_socio_id, v_sede_id, v_nombre, v_foto
