-- Lead convertido = lead CERRADO. Dos huecos que dejaban leads "Socio (check)"
-- varados en Clase de prueba con tareas vencidas eternas en la agenda:
--   1) inscribir_socio marcaba etapa='inscrito' pero NO completaba las tareas
--      de seguimiento pendientes del lead.
--   2) una inscripcion directa (sin pasar por el CRM) no enlazaba el lead
--      abierto de la misma persona (match por documento).
-- Ademas, saneo de datos: leads que ya tienen socio_id pasan a 'inscrito' y
-- sus tareas abiertas se completan.

CREATE OR REPLACE FUNCTION public.inscribir_socio(p_sede_id uuid, p_nombre text, p_telefono text DEFAULT NULL::text, p_email text DEFAULT NULL::text, p_documento text DEFAULT NULL::text, p_fecha_nacimiento date DEFAULT NULL::date, p_objetivo text DEFAULT NULL::text, p_plan_id uuid DEFAULT NULL::uuid, p_lead_id uuid DEFAULT NULL::uuid, p_promocion_id uuid DEFAULT NULL::uuid, p_metodo_pago text DEFAULT 'efectivo'::text, p_invitados jsonb DEFAULT NULL::jsonb, p_monto_inicial numeric DEFAULT NULL::numeric, p_precio_acordado numeric DEFAULT NULL::numeric, p_objetivo_id uuid DEFAULT NULL::uuid, p_peso_kg numeric DEFAULT NULL::numeric, p_talla_m numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  insert into public.socio (empresa_id, sede_id, codigo, nombre, telefono, email, documento, fecha_nacimiento, objetivo, objetivo_id, peso_kg, talla_m, estado, created_by)
...
-- Saneo de datos existentes (demo y reales):
update public.lead set etapa = 'inscrito' where socio_id is not null and etapa <> 'inscrito' and deleted_at is null;
update public.lead_tarea t set completada = true
  from public.lead l
  where l.id = t.lead_id and l.socio_id is not null and not t.completada;
